...
// #define BRIGHTNESS  64     // never referenced anywhere;
...
// --- Effect Mode Enum --- // 25 base modes + 12 combos + 5 wheels = 42 total
enum EffectMode {  /// Define the effect modes
  ...
  FlowerMono,      // Flower Outline (Static) in a single color
...
unsigned long getDynamicPhaseDuration() {
  switch (currentEffect) {
    ...
    // case CoolPinwheel:        return 300000;  // 5m
    case SparkInvaders:       return 120000;  // 2m
    case SpacePortal:         return 600000;  // 10m default
    case FlowerFocus:         return 180000;  // 3m
    case SunTarget:           return 180000;  // 3m
    case IsasFireworks:       return 120000;  // 2m
    case KunterbuntSpiral:    return 180000;  // 3m
    // case StargateSG1:         return 300000;  // 5m
    case LatEotT:            return 300000;  // 5m
  
    // // New wheel modes
    // case WheelTurn:           return 600000;  // 10m default
    // case RainbowWheel:        return 600000;  // 10m default
    // case CottonCandy:         return 600000;  // 10m default
    // case SeaAndSky:           return 600000;  // 10m default
    // case FieryWheel:          return 600000;  // 10m default 
...
// Softness palette: soft-orange, orange, red, pink; doubled
...
bool firstCirclesWipeRun = true;    // Flag to track first run of Circles Wipe effect
...
void showFlowerMono(CRGB inputColor) { // Monochromatic variant of FlowerOutline()
  ...
  for (int c = 0; c < 8; c++) {
    int p = (c + 7) % 8;
    for (int i = 0; i < 6; i++) {
      leds[leftCurves[(p)%8][i]] = color;
      if (!(c == 7 && (i == 0 || i == 1))) {
        leds[rightCurves[(c)%8][i]] = color;
      }
    }
  }
  // FastLED.show();
...
void showFragileSpokes(int octalRotations) {  // Revisited, reconsidered, and left static
  ...
  for (int r = 0; r < 8; r++) { 
    ...
    for (int j = 1; j <= 4; j++) {
        leds[rightCurves[(r + octalRotations) % 8][j]] = color;
        leds[leftCurves[(r + octalRotations + 7) % 8][j]] = color;
    }
  }
...
void showCirclesWipe() {  // On base MVM- toggle pixels clockwise in random rings
  ...
  if (firstCirclesWipeRun) {
    memset(pixelToggled, 0, sizeof(pixelToggled));
    currentRing = -1;
    currentPixel = 0;
    firstCirclesWipeRun = false;
  }
...
static uint8_t       rainbowWheelHue        = 0;
static unsigned long rainbowWheelLastUpdate = 0;
void showRainbowWheel(uint16_t fadeSpeed) {
  // Update every fadeSpeed ms for smooth animation
  if (millis() - rainbowWheelLastUpdate > fadeSpeed) { 
    rainbowWheelLastUpdate = millis();
    rainbowWheelHue++;
  }
  for (int a = 0; a < 64; a++) {
    // Compute hue for this angle group, cycling through the full colorspace
    uint8_t angleHue = (rainbowWheelHue + a * 4) % 256; // 64*4=256 covers full hue wheel
    // uint8_t angleHue = rainbowWheelHue; // removes angular dependence, just cycles the whole mandala's hue
    for (int i = 0; i < angleLens[a]; i++) {
...
void showRainbowClock() {
  ...
  if (museGetTime(h, m, s)) {
    ...
    // }
    static bool secondHandSynced = false;
    if (!secondHandSynced) {
      int reversedSec = (60 - s) % 60;                              // flip direction to run clockwise
      int syncSlice   = (((reversedSec * 64 + 30) / 60) + 16) % 64;  // nearest 1/64th slice, top-aligned
      rainbowWheelHue = (256 - syncSlice * 4) % 256;                 // put hue 0 (red) there, right now
      secondHandSynced = true;                                      // one-time alignment only
    }
    // h=0; m=52; s=30; // min hand in NW position for testing; comment out to use real time

    int elapsedMin  = m * 60 + s;                     // seconds into the current hour, 0-3599
    int reversedMin = (3600 - elapsedMin) % 3600;      // flip direction to run clockwise
    int minuteAngle = (((reversedMin * 64 + 1800) / 3600) + 16) % 64; // nearest 1/64th slice, top-aligned
    leds[anglePixels[minuteAngle][angleLens[minuteAngle] - 1]] = CRGB::White; // outermost pixel

    int elapsedHr  = (h % 12) * 3600 + m * 60 + s;     // seconds into the current 12h cycle, 0-43199
    int reversedHr = (43200 - elapsedHr) % 43200;      // flip direction to run clockwise
    int hourSpoke  = (((reversedHr * 16 + 21600) / 43200) + 4) % 16; // nearest 1/16th spoke, top-aligned
    ...
    if (hourSpoke % 2 == 1) { // secondary spoke: flank the tip to match the cardinals' 5-pixel reach
      ...
      leds[anglePixels[(a + 63) % 64][0]] = CRGB::White; // one slice back (ring5)
      leds[anglePixels[(a + 1) % 64][0]]  = CRGB::White; // one slice forward (ring5)
    }
  }
}
...
void showCoolPinwheel() { // order flipped; Isa agrees it's better
...
void showIsasFireworks() { // repaired; Isa approves
  showAperture();
  showTarget(CRGB::Magenta);
  showCenterBurst();
  showTwinkleReal();
...
void showHornySeason() { // good and approved
...
void setup() {
  ...
  // while (!Serial && millis() - serialWait < 2000) delay(10); // let native USB CDC enumerate
  ...
  museTimeBegin(); // Attempt WiFi/NTP sync (up to 2 tries, then gives up)
}

// --- Main loop ---
void loop() {
  museTimeTick(); // non-blocking after first sync; drives WiFi retry/resync schedule
  ...
  if (altMandalaActive && millis() - lastTouch <= getDynamicPhaseDuration()) {
    ...
    switch (currentEffect) {
      ...
      case FlowerMono:     showFlowerMono(yearPalette[2]); break; // Chartreuse (Ostara) flower
      ...
      case CottonCandy:         showWheel64(pastelPalette, 24); break; // rp2.5s
      case SeaAndSky:           showWheel64(coolPalette, 20); break; // rp3s
      case FieryWheel:          showWheel64(softPalette, 12); break; // rp5s
      case Clock:               showRainbowClock(); break; // real-time rainbow clock
