...
// --- Effect Mode Enum --- // 26 of them! 18+2 active
enum EffectMode {  /// Define the effect modes
  ...
  RainbowOut,      // Rainbow fade (Dynamic)
...
// // Helper for WOTYCycle: Calculate safe hue delta that avoids green zone (hue 96-128)
// int16_t safeHueDelta(uint8_t startHue, uint8_t endHue) {
//   // Calculate both directions around the wheel
//   int16_t forwardDelta = (int16_t)endHue - (int16_t)startHue;
//   if (forwardDelta < 0) forwardDelta += 256;
  
//   int16_t backwardDelta = (int16_t)startHue - (int16_t)endHue;
//   if (backwardDelta < 0) backwardDelta += 256;
//   backwardDelta = -backwardDelta; // Negative for backward direction
  
//   // Define green zone: hue 96-128 (pure green vicinity)
//   const uint8_t greenStart = 96;
//   const uint8_t greenEnd = 128;
  
//   // Check if forward path crosses green zone
//   bool forwardCrossesGreen = false;
//   for (int16_t h = 0; h < forwardDelta; h++) {
//     uint8_t testHue = (startHue + h) & 0xFF;
//     if (testHue >= greenStart && testHue <= greenEnd) {
//       forwardCrossesGreen = true;
//       break;
//     }
//   }
  
//   // If forward crosses green, use backward; otherwise use shorter path
//   if (forwardCrossesGreen) {
//     return backwardDelta;
//   } else {
//     // Use whichever is shorter
//     return (abs(forwardDelta) <= abs(backwardDelta)) ? forwardDelta : backwardDelta;
//   }
// }
...
// static int relayDebounceCount = 0; // New: Counter for stable readings
// const int requiredStableReads = 10; // Require 10 consecutive stable reads (adjust as needed)

const unsigned long debounceDelay = 100; // 100ms debounce
// const unsigned long relayDebounceDelay = 1000; // Increased to 1000ms for relay
// const unsigned long minHoldTime = 50; // Minimum time the button must be held to register (ms)
...
void showAperture() { // For the good of all of us
  ...
  if (firstRun || millis() - lastUpdate > updateInterval) {
    if (firstRun) {
      firstRun = false;
      fill_solid(leds, NUM_LEDS, CRGB::Black);
    }
  ...
  // for (int i = 0; i < ringSizes[0]; i++) leds[ring1[i]] = ring9Color;
  // for (int i = 0; i < ringSizes[1]; i++) leds[ring2[i]] = ring8Color;
...
void showWOTYRotate() {
  ...
  for (int s = 0; s < 16; s++) {
    ...
    int paletteIndex = ((s + rotationOffset) / 2) % 8;
    CRGB color = yearPalette[7 - paletteIndex]; 
...
void loop() {
  ...
  if (altMandalaActive && millis() - lastTouch <= getDynamicPhaseDuration()) {
    ...
    switch (currentEffect) {
      ...
      case WOTYRotate:     showWOTYRotate(); break;
    ...
    // showWOTY(320); // Nov 16th
    // showWOTYRotate(); // Rotate the WOTY during standby

    // Snowy Winter Solstice standby:
